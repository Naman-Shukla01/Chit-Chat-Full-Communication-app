...
const AiChatWindow = ({ user, showProfile, setShowProfile }) => {
  ...
  return (
    ...
    ></div>
      ...
      <div
        className="p-8 pl-16 not-sm:pl-0 not-sm:ml-4 w-full pb-32 h-[85vh] overflow-y-auto space-y-3   
  ...
  <span className="ml-1 text-transparent bg-clip-text bg-gradient-to-b from-blue-500  via-purple-500 to-pink-500 hover:brightness-110 transition-all duration-300 ease-in-out">
   Ask <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 underline decoration-[#89AAFF] underline-offset-4 transition-all duration-300 ease-in-out hover:brightness-110">
  anything.
</span></span>
  ...
  <div className="w-2 h-3 bg-orange-400 rounded-full animate-bounce"></div>
        ...
        <form
          onSubmit={sendPrompt}
          className="w-full flex not-sm:mr-2 sm:ml-14 fixed bottom-8 not-sm:bottom-20 "
        >
          <input
            type="text"
            name="message"
            ...
            className="p-2 text-sm border-2 not-sm:w-[80vw] w-[40vw] border-[#F7B264] rounded-full bg-white mr-2"
          />
          <button
            type="submit"
